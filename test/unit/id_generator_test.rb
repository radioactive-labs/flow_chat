...
module FlowChat
  class IdGeneratorTest < Minitest::Test
    ...
    def test_id_within_256_char_limit
      ...
      assert id.length <= IdGenerator::DEFAULT_MAX_ID_LENGTH
    end
    ...
    def test_duplicate_long_label_within_limit
      ...
      assert id1.length <= IdGenerator::DEFAULT_MAX_ID_LENGTH
      assert id2.length <= IdGenerator::DEFAULT_MAX_ID_LENGTH
      refute_equal id1, id2
    end
    ...
    def test_max_length_without_hash_suffix
      ...
      label = "a" * IdGenerator::DEFAULT_MAX_ID_LENGTH
      id = @generator.generate_id(label)

      assert_equal IdGenerator::DEFAULT_MAX_ID_LENGTH, id.length
      assert_equal "a" * IdGenerator::DEFAULT_MAX_ID_LENGTH, id
    end

    def test_max_length_with_hash_suffix
      # Create a duplicate that needs hash suffix
      long_label = "a" * IdGenerator::DEFAULT_MAX_ID_LENGTH
      @generator.generate_id(long_label)
      id_with_hash = @generator.generate_id(long_label)

      assert id_with_hash.length <= IdGenerator::DEFAULT_MAX_ID_LENGTH
      # Hash suffix uses space separator and 3-char hash
    ...
    def test_max_length_is_configurable
      generator = IdGenerator.new(max_length: 10)

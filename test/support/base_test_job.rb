...
class BaseTestJob
  ...
  def self.perform_later(*args)
    job_data = { class: self.name, args: args, performed_at: Time.now }
  ...
  def self.create_mock_whatsapp_client
    ...
    client.define_singleton_method(:send_message) { |phone, message| sent_messages << [phone, message]; { "messages" => [{ "id" => "test_#{rand(1000)}" }] } }
    client.define_singleton_method(:send_text) { |phone, text| sent_messages << [phone, [:text, text]]; true }
